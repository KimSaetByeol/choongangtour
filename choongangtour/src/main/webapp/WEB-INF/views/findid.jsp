...
function check(){
	...
	$.ajax({
		...
		cache : false,
		dataType : "html",
		data : {'l_name' : l_name, 'l_email' : l_email},
...
<div id ="middle"  align="center"   style=" padding-top: 20px; padding-bottom: 30px; display:flex; flex-direction: row; justify-content: space-evenly;">
  ...
  <div id ="article" align="center" style=" box-shadow: 1px 1px 1px 1px #C9C9C9; border-style: outset; border-radius: 10px; padding: 5px;  border-radius: 10px; padding-top: 10px;   width: 1150px; ">
	<div>
	...
	<c:if test="${myinfo ne null }">
		찾으신 ID는 ${myinfo.l_id } 입니다.<br><br>
		비밀번호를 바꾸려면 닉네임을 입력하세요.<br>
		<form action="./changeInfo.do" method="post">
			<input type="hidden" name="code" value="changepw">
			<input type="hidden" name="l_no" value="${myinfo.l_no }">
			email : <input type="email" name="l_email" id="l_email" value="${myinfo.l_email }" disabled="disabled"><br>
			name : <input type="text" name="l_name" id="l_name" required="required" onchange="return check()"><br>
			<p id="checkResult"></p>
